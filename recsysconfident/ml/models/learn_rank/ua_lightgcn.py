"""
Using implementation of LightGCN combined with confidence estimation from UAGAT
"""
...
def get_ualightgcn_model_and_dataloader(info: DatasetInfo):
...
class UALightGCN(TorchModel):
    ...
    def __init__(self, items_per_user, Graph, n_users: int, n_items: int, emb_dim: int, n_layers: int, keep_prob: float, A_split, rmin, rmax, dropout=True):
        super(UALightGCN, self).__init__(items_per_user, None, n_users, n_items, emb_dim)
        ...
        print("UA Light GCN instantiated")
    ...
    def forward(self, users, items):
        ...
        mean = torch.sum(inner_pro, dim=1)

        # Softplus/clamp ensures γ > 0
        gamma_u = torch.clamp(self.user_gamma(users), min=0.00001)
    ...
    def nll_bpr_loss(self, user_ids, item_ids):
        ...
        high_sigma = high_scores[:, 1].clamp(min=1e-6)
